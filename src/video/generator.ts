...
import { RunResult, StepResult } from '../types.js';
 ...
 */
export async function generateVideo(result: RunResult, options: VideoOptions = {}): Promise<string | null> {
  const {
    frameDuration = 2,
    width = 1280,
    fadeDuration = 0.3,
...
function assembleOneFrame(
  ...
  try {
    execSync(
      `ffmpeg -y -loop 1 -i "${framePath}" -t ${duration} ` +
...
function assembleWithTransitions(
  frames: string[],
  outputPath: string,
  frameDuration: number,
  width: number,
  ...
  const scaleFilters = frames
    .map((_, i) =>
      `[${i}:v]scale=${width}:-2:flags=lanczos,pad=${width}:${maxHeight}:(ow-iw)/2:(oh-ih)/2:black,setsar=1,fps=30[v${i}]`)
    .join('; ');
  ...
  if (frames.length === 2) {
    ...
    for (let i = 1; i < frames.length; i++) {
      ...
      xfades.push(
        `[${prevLabel}][v${i}]xfade=transition=fade:duration=${fadeDuration}:offset=${offset}${
          i === frames.length - 1 ? ',format=yuv420p' : ''
        }[${outLabel}]`,
      );
      prevLabel = outLabel;
    }
