import { createHash } from 'crypto';
import { existsSync, mkdirSync, readFileSync, writeFileSync } from 'fs';
import { join } from 'path';
const CACHE_DIR = '.accept/cache';
function ensureCacheDir() {
    if (!existsSync(CACHE_DIR)) {
        mkdirSync(CACHE_DIR, { recursive: true });
    }
}
function cacheKey(stepDescription, a11ySnapshot) {
    const hash = createHash('sha256');
    hash.update(stepDescription);
    hash.update(a11ySnapshot);
    return hash.digest('hex').slice(0, 16);
}
export function getCached(stepDescription, a11ySnapshot) {
    const key = cacheKey(stepDescription, a11ySnapshot);
    const path = join(CACHE_DIR, `${key}.json`);
    if (!existsSync(path)) {
        return null;
    }
    try {
        const data = readFileSync(path, 'utf-8');
        return JSON.parse(data);
    }
    catch {
        return null;
    }
}
export function setCache(stepDescription, a11ySnapshot, entry) {
    ensureCacheDir();
    const key = cacheKey(stepDescription, a11ySnapshot);
    const path = join(CACHE_DIR, `${key}.json`);
    const fullEntry = {
        ...entry,
        timestamp: new Date().toISOString(),
    };
    writeFileSync(path, JSON.stringify(fullEntry, null, 2));
}
//# sourceMappingURL=cache.js.map