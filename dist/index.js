...
import { runSpec } from './runner/explore.js';
import { saveRunResult, cleanupOldRuns } from './evidence/collector.js';
...
program
    ...
    const skillContent = readFileSync(new URL('../../skill/accept.md', import.meta.url), 'utf-8').toString();
            ...
            for (const entry of entries) {
                ...
                if (existsSync(resultsPath)) {
                    try {
                        ...
                        const status = data.failed === 0 ? chalk.green('PASS') : chalk.red('FAIL');
                        const time = new Date(data.timestamp).toLocaleString();
                        const shareInfo = data.shareUrl ? chalk.cyan(` → ${data.shareUrl}`) : '';
                        console.log(`  #${entry} ${status} ${data.spec?.title || 'Untitled'} (${time})${shareInfo}`);
                    }
                    catch {
    ...
    if (options.cloud) {
        ...
        if (cloudRuns.length === 0) {
            ...
            for (const run of cloudRuns) {
                const status = run.failed === 0 ? chalk.green('PASS') : chalk.red('FAIL');
                const time = new Date(run.timestamp).toLocaleString();
                console.log(`  ${status} ${run.title} (${time}) ${chalk.cyan(run.url)}`);
            }
        }
    }
